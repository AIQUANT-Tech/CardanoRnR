...
const ReviewsPage = () => {
  ...
  // Function to fetch review data from the backend.
  const fetchReviewData = async () => {
    try {
      ...
      if (result.review_rating_fetch_rs) {
        let repScore = result.review_rating_fetch_rs.reputation_score;
        // If API returns 0 or falsy, try to load a cached reputation score.
        if (!repScore || repScore === 0) {
          const cachedScore = localStorage.getItem("reputationScore");
          if (cachedScore) {
            repScore = parseFloat(cachedScore);
          }
        } else {
          // When API returns a valid reputation score, update localStorage.
          localStorage.setItem("reputationScore", repScore);
        }
        result.review_rating_fetch_rs.reputation_score = repScore;
      }
      setData(result.review_rating_fetch_rs);
      setLoading(false);
    } catch (error) {
  ...
  // Poll every 10 seconds for updated reputation score.
  useEffect(() => {
    if (data && Number(data.reputation_score) > 0) {
      return;
    }
  ...
  // Show either all categories or just the first three.
  const visibleCategories = showAllCategories
    ? category_wise_review_rating
    : category_wise_review_rating.slice(0, 3);
        ...
        <Grid container spacing={4}>
          <Grid item xs={12} md={4}>
            ...
            >
              ...
              <Typography variant="h2" fontWeight="bold" color="#333">
                {parseFloat(reputation_score).toFixed(2)}
                <br />
                /100
              </Typography>
            </Box>
